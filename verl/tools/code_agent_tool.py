...
class CodeAgentExecutionWorker:
...
def init_code_agent_execution_pool(
    num_workers: int, enable_global_rate_limit=True, rate_limit=10, mode: PoolMode = PoolMode.ThreadMode
):
    """Initialize search execution pool."""
    if mode == PoolMode.ThreadMode:
        return (
            ray.remote(CodeAgentExecutionWorker)
...
class CodeAgentTool(BaseTool):
    """Code agent tool for retrieving information using external code agent services.
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        self.execution_pool = init_code_agent_execution_pool(
